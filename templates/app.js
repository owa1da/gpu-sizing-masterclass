...
(function(){
  ...
  function setupObservers(){
    const revO=new IntersectionObserver((es)=>{
      es.forEach(e=>{ if(e.isIntersecting){e.target.classList.add("in"); revO.unobserve(e.target);} });
    ...
    const secO=new IntersectionObserver((es)=>{
      es.forEach(e=>{
        if(e.isIntersecting){
          ...
          if(e.target.dataset.earnBadge) earn(e.target.dataset.earnBadge); // a section can unlock a badge via data-earn-badge="id"
        }
      });
    },{threshold:.5});
